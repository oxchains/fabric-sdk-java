...
public class EventHub {
    ...
    synchronized boolean connect(final TransactionContext transactionContext) throws EventHubException {
        ...
        managedChannelHandle = new Endpoint(url, properties).getHandle();

        events = EventsGrpc.newStub(managedChannelHandle.getManagedChannel());
        ...
        StreamObserver<PeerEvents.Event> eventStream = new StreamObserver<PeerEvents.Event>() {
            ...
            public void onError(Throwable t) {
                ...
                final boolean isTerminated = managedChannelHandle.isTerminated();
                final boolean isChannelShutdown = managedChannelHandle.isShutdown();
                    ...
                    if (sre.getStatus().getCode() == Status.Code.INTERNAL) {
                        ...
                        try {
                            if (!isChannelShutdown) {
                            	managedChannelHandle.shutdownNow();
                            }
    ...
    public void shutdown() {
        ...
        new Endpoint(url).closeManagedChannel(managedChannelHandle);

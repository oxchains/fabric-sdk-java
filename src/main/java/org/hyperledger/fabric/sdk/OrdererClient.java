...
class OrdererClient {
     ...
     */
    OrdererClient(Orderer orderer, ManagedChannelHandle managedChannelHandle) {
    ...
    synchronized void shutdown(boolean force) {
        ...
        shutdown = true;
        ManagedChannelHandle handle1 = managedChannelHandle;
        managedChannelHandle = null;
        if (handle1 == null) {
            return;
        }
        if (force) {
             new Endpoint(url).closeManagedChannel(handle1);
        } else {
            boolean isTerminated = false;

            try {
                isTerminated = handle1.shutdown().awaitTermination(3, TimeUnit.SECONDS);
            } catch (Exception e) {
    ...
    Ab.BroadcastResponse sendTransaction(Common.Envelope envelope) throws Exception {
		...
		if(managedChannelHandle == null || managedChannelHandle.isTerminated() || managedChannelHandle.isShutdown()){
			managedChannelHandle = new Endpoint(url).getHandle();
		}
        AtomicBroadcastGrpc.AtomicBroadcastStub broadcast = AtomicBroadcastGrpc.newStub(managedChannelHandle.getManagedChannel());
        AtomicBroadcastGrpc.AtomicBroadcastBlockingStub bsc = AtomicBroadcastGrpc.newBlockingStub(managedChannelHandle.getManagedChannel());
